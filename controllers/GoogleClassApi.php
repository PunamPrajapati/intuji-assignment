...
class GoogleCalendarApi { 
    ...
    public function UnsyncCalendarEvent($access_token) { 
        $apiURL = 'https://oauth2.googleapis.com/revoke?token='.$access_token;         
        ...
        if (!$response) { 
            $error_msg = 'Failed to disconnect Google Calendar'; 
            if (curl_errno($ch)) { 
                $error_msg = curl_error($ch); 
            } 
            throw new Exception('Error '.$http_code.': '.$error_msg); 
